...
def download_from_gdrive(file_id, output_path):
    ...
    """
    try:
        # Пробуем использовать gdown для загрузки
        try:
            import gdown
        ...
        # Альтернативный метод: использование wget
        try:
            print("Попытка загрузки с использованием wget...")
            subprocess.run([
            ...
            print(f"Ошибка при загрузке с помощью wget: {wget_e}")
            return False
...
def main():
    ...
    # 1. Сначала проверим наличие CSV файла на Google Drive
    gdrive_file_id = "1WVMvdjL6Q_vUD_Lb-jbEfPaDrT7pEerf"  # ID файла на Google Drive
    
    if not os.path.exists(target_csv):
        print("Попытка загрузки данных с Google Drive...")
        if download_from_gdrive(gdrive_file_id, str(target_csv)):
            print(f"Файл успешно загружен с Google Drive и сохранен в {target_csv}")
        else:
            # 2. Если загрузка с Google Drive не удалась, ищем локальные пути
            print("Загрузка с Google Drive не удалась. Проверка локальных путей...")
    ...
    print("python -m mushroom_classifier.data")
